...
export function ComponentCard({ component }: { component: Component }) {
  ...
  return (
    <div className="border border-black/[.08] dark:border-white/[.145] rounded-lg bg-white dark:bg-zinc-950 overflow-hidden">
      <button
        className="w-full flex items-center justify-between px-4 py-3 bg-zinc-50 dark:bg-zinc-900 text-left"
        onClick={() => setOpen((o) => !o)}
      >
        <span className="font-medium text-sm">{component.name}</span>
        <span className="text-xs text-zinc-500 dark:text-zinc-400">{open ? "▴" : "▾"}</span>
      </button>
      {open && (
        <div className="flex flex-col lg:flex-row">
          <div className="p-4 space-y-3 lg:w-[420px] lg:min-w-[420px] lg:border-r border-black/[.08] dark:border-white/[.145]">
            <div>
            ...
            <div>
              ...
              <textarea
                className="w-full h-32 text-xs font-mono border border-black/[.08] dark:border-white/[.145] rounded-md px-2 py-1.5 bg-transparent"
          ...
          <div className="flex-1 min-w-0 flex flex-col bg-zinc-100 dark:bg-zinc-900">
            <div className="px-4 py-2.5 border-b border-black/[.08] dark:border-white/[.145] flex items-center justify-between bg-white dark:bg-zinc-950">
            ...
            <div className="flex-1 overflow-auto p-5 flex justify-center">
              <iframe
                ...
                style={{ minHeight: "260px", width: widthPx, maxWidth: "100%" }}
                srcDoc={previewHtml}
              />
            </div>
          </div>
