...
// verschwinden.
//
// Tokens sind Punkt-Pfade in ein beliebig verschachteltes JSON-Objekt,
// z.B. {{TICKET.CREATE.TITLE}} -> tokens.TICKET.CREATE.TITLE. Namenskonvention:
// <ENTITÄT>.<EREIGNIS>.<FELD> (z.B. TICKET.CREATE.TITLE, AUTH.LOGIN.URL) —
// bewusst ohne feste Whitelist im Code, damit n8n neue Felder mitschicken
// kann, ohne dass hier etwas angepasst werden muss.
export function substituteRuntimeTokens(text, tokens) {
  return String(text ?? "").replace(/\{\{([A-Z0-9_]+(?:\.[A-Z0-9_]+)*)\}\}/g, (match, path) => {
    const value = path.split(".").reduce((obj, key) => (obj == null ? undefined : obj[key]), tokens);
    return value !== undefined && value !== null ? String(value) : match;
  });
}

// Baut den kompletten Mail-HTML-Body (ohne <head>/<style>-Wrapper) aus
// Vorlage + aufgeloesten Bloecken. Ein Banner ist hier keine strukturelle
// Sonderrolle mehr, sondern einfach ein "Bild"-Baustein wie jeder andere —
// wer eine Vorlage mit Banner braucht, fuegt ihn ueber "+ Block" hinzu.
// Footer bleibt die eine strukturelle Ausnahme (per Namen gefunden), weil
// er ueber den eigenen Vorlagen-Schalter "Footer aktiv" gesteuert wird statt
// Teil der umsortierbaren Bloecke zu sein. Der reine Dokument-Wrapper
// (Fonts, Media-Queries, Outlook-Fallbacks) bleibt konstant und kein
// Notion-Inhalt.
...
export function sampleContentForComponent(component) {
  ...
  for (const field of component.fieldSchema) {
    ...
    } else if (field.type === "url" && field.key === "src") {
      // Bildfelder (z.B. die "Bild"-Komponente) brauchen eine echte,
      // aufloesbare Bild-URL fuer die Vorschau, keinen Seiten-Link.
      content[field.key] = "https://assets.stadtteilliebe.de/peace.jpg";
    } else if (field.type === "url") {
...
// Isolierte Vorschau eines einzelnen Bausteins (ohne Footer) — nutzt
...
export function renderComponentPreview(component, content) {
  const fakeTemplate = { footerEnabled: false };
